...
已有 TRL + PEFT LoRA/QLoRA DPO 训练器、数据契约校验和独立单元测试。当前处于阶段 B：已缓存原神三角色文本，实现无标签开发快照、离线裁判请求/解析及[36对人工校准包设计](docs/GENSHIN_STAGE_B_CALIBRATION.md)；语义分组、正式划分冻结与实际裁判校准仍待完成。随后优先基座/GRPO，再补 DPO 对照。

尚未发布正式数据集或效果结果；GRPO、真实裁判调用及正式统一评测尚未完成。微型训练只能验证软件机制，不代表角色能力提升。
