...
第二条正常时输出 `status=ok`；这只验证连通性和JSON格式，`calibration_passed`仍为false。`runs/api_smoke_01/call/`保存请求、原始模型返回、用量及严格解析结果。404需检查API路径/模型ID，401/403检查密钥及账户权限，400检查接口支持的参数；不得自动换模型或补造必需字段。JSON mode关闭时仍执行本地严格解析。需要进一步诊断时提供脱敏失败记录，不发送`.env`。
...
真实训练的命令结构如下，**这些数据目录尚未生成，不能直接复制为已就绪训练**：
...
CUDA_VISIBLE_DEVICES=0,1 torchrun --standalone --nproc_per_node=2 scripts/train_grpo.py \
  ...
  --suite runs/agent_training_suite_01 \
  --packet runs/agent_calibration_packet_01 \
  --judge-results runs/agent_judge_calibration_01/calls \
  --human runs/agent_human_results_01 \
  --steps 2 --output runs/qwen3_8b_grpo_01
```

先添加`--preflight-only`检查数据和真实校准；该模式不请求API、不加载模型、不更新参数。移除后才训练。策略采用FP16基座+FP32 LoRA训练参数，TRL处理混合精度；每卡microbatch=1、累积2步、每组2样本。每进程生成一组，同步全局奖励和梯度；双卡各持有完整8B基座，不合并成64GB显存。单卡32GB的实际峰值仍需测量。

按rank独立保存请求/返回/适配器，避免双进程覆盖。每个进程100次API尝试上限包含重试，两进程最多200次；超时120秒/次。奖励异常先跨进程同步再同时拒绝更新，避免一边反向传播、一边已退出。失败仍留档；同分组不虚造学习信号。完整任务训练前后比较仍需用Agent轨迹，当前优化单位为单步动作。

下一阶段还需要生成本设计任务的开发训练输入/校准材料、实际填写人审并验收API裁判。现有原神36对不扩充、不代填，也不能授权本任务奖励。`runs/`、旧人审包和模型不随Git下载。当前可以直接执行步骤1—5中的环境、配置、单请求、基线和CPU双进程检查；真实Qwen训练等待上述输入就绪。
